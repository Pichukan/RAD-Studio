...
var
  ...
  WRD,Book,BookPicture,BookAnalogiCoInvest,BookObzor,BookAnalog,BookAnalogBuild,BookLocation,BookFoto,
...
//**** Процедура ищет из таблицы EXCEL название района и сегмента рынка   *****
...
//**** и вставляет в документ WORD отчета                                 *****
 ...
 begin
      ...
      RangeObzor:=MyWorkSheet2.Range['c4'];
      ObzorSegment:= RangeObzor.Value;

   //************************ Обзор района *******************************

        DefaultReadIniFile := 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОРЫ районы\';   //Значение директории обзора по умолчанию
  ...
  //********************  Обзор экономики РФ*************************
        ...
        DefaultReadIniFile := 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОР ЭКОНОМИКИ\';   //Значение директории обзора по умолчанию
        DirObzor:= IniFile.ReadString(IniSectionValue [1,1], IniSectionValue [3,2], DefaultReadIniFile)+'обзор экономики РФ.docx';
      ...
      BookObzor:=WRD.Documents.Open(DirObzor);
         ...
         ShowMessage('BLEVANTONE не смог открыть файл с обзорами.'+
       ' Пожалуйста укажите расположение документа с обзором экономики РФ... '+
        ' ну или, если нет, любого другого ;)');
        ...
        if not Form1.OpenDialog1.Execute then Exit;  //тут открывается диалог выбора файла, и если пользователь нажал "Cancel", то выходим
         ...
         IniSectionValue [3,3]:= DIRFileDetect2(DirObzor);  //отсечение имени файла с обзором района от пути
         IniFile.WriteString(IniSectionValue [1,1], IniSectionValue [3,2], IniSectionValue [3,3]);

      end;

      BookObzor.Range.Copy;
      ReplaceTextObzor:='1###RF1';
      MyRange2 := FindInDoc(Book, ReplaceTextObzor);

         if VarIsClear(MyRange2) then begin
            ShowMessage('Текст 1###RF1 ReplaceTextObzor НЕ найден.');
  //          Exit;
         end;
  ...
  //********************  Обзор экономики СПб и ЛО *************************

        DefaultReadIniFile := 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОР ЭКОНОМИКИ\';   //Значение директории обзора по умолчанию
        DirObzor:= IniFile.ReadString(IniSectionValue [1,1], IniSectionValue [4,2], DefaultReadIniFile)+'обзор экономики СПб и ЛО.docx';
      ...
      BookObzor:=WRD.Documents.Open(DirObzor);
         ...
         ShowMessage('BLEVANTONE не смог открыть файл с обзорами.'+
       ' Пожалуйста укажите расположение документа с обзором экономики СПб и ЛО... '+
        ' ну или, если нет, любого другого ;)');
        ...
        if not Form1.OpenDialog1.Execute then Exit;  //тут открывается диалог выбора файла, и если пользователь нажал "Cancel", то выходим
         ...
         IniSectionValue [4,3]:= DIRFileDetect2(DirObzor);  //отсечение имени файла с обзором района от пути
         IniFile.WriteString(IniSectionValue [1,1], IniSectionValue [4,2], IniSectionValue [4,3]);

      end;

      BookObzor.Range.Copy;
      ReplaceTextObzor:='1###SPB1';
      MyRange2 := FindInDoc(Book, ReplaceTextObzor);
...
//********************  Обзор экономики сегмент рынка *************************

        DefaultReadIniFile := 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОР СЕГМЕНТ РЫНКА\';   //Значение директории обзора по умолчанию
        DirObzor:= IniFile.ReadString(IniSectionValue [1,1], IniSectionValue [5,2], DefaultReadIniFile)+'обзор '+ObzorSegment+'.docx';
      ...
      BookObzor:=WRD.Documents.Open(DirObzor);
         ...
         ShowMessage('BLEVANTONE не смог открыть файл с обзорами.'+
       ' Пожалуйста укажите расположение документа с обзором экономики сегмента рынка ОО... '+
        ' ну или, если нет, любого другого ;)');
        ...
        if not Form1.OpenDialog1.Execute then Exit;  //тут открывается диалог выбора файла, и если пользователь нажал "Cancel", то выходим
         ...
         IniSectionValue [5,3]:= DIRFileDetect2(DirObzor);  //отсечение имени файла с обзором района от пути
         IniFile.WriteString(IniSectionValue [1,1], IniSectionValue [5,2], IniSectionValue [5,3]);

      end;

      BookObzor.Range.Copy;
      ReplaceTextObzor:='1###SEG1';
      MyRange2 := FindInDoc(Book, ReplaceTextObzor);

         if VarIsClear(MyRange2) then begin
            ShowMessage('Текст 1###SEG1 ReplaceTextObzor НЕ найден.');
  //          Exit;
         end;
 ...
 //**** Процедура ищет из таблицы EXCEL название ОЦЕНОЧНОЙ КОМПАНИИ и       *****
 //*** открывает документ WORD с документами компании  (страховка и др.)    *****
//**** и вставляет в документ WORD отчета                                   *****
 procedure InsertDocCompany();
 var
    DirObzor,ReplaceTextObzor : string;
    DefaultReadIniFile        : string;
    DociCompany               : string;
 ...
 begin
      ...
      RangeObzor:=MyWorkSheet2.Range['c3'];
      DociCompany:= RangeObzor.Value;

   //************************ Обзор района *******************************

        DefaultReadIniFile := 'Z:\GRAND NEVA\2014\КОМПАНИИ\ГРАНД НЕВА\';   //Значение директории обзора по умолчанию
        DirObzor:= IniFile.ReadString(IniSectionValue [1,1], IniSectionValue [6,2], DefaultReadIniFile)+DociCompany+' документы компании и оценщиков.docx';

      try
      BookObzor:=WRD.Documents.Open(DirObzor);
      except

         ShowMessage('BLEVANTONE не смог открыть файл с доками компании.'+
       ' Пожалуйста укажите расположение документа с доками компании '+
        DociCompany+' ну или, если нет, какой там надо млин-н... ;)');
        ...
        if not Form1.OpenDialog1.Execute then Exit;  //тут открывается диалог выбора файла, и если пользователь нажал "Cancel", то выходим
         ...
         IniSectionValue [6,3]:= DIRFileDetect2(DirObzor);  //отсечение имени файла с обзором района от пути
         IniFile.WriteString(IniSectionValue [1,1], IniSectionValue [6,2], IniSectionValue [6,3]);

      end;

      BookObzor.Range.Copy;
      ReplaceTextObzor:='1###DOCCOMP1';
      MyRange2 := FindInDoc(Book, ReplaceTextObzor);

         if VarIsClear(MyRange2) then begin
            ShowMessage('Текст 1###DOCCOMP1 ReplaceTextObzor НЕ найден.');
  //          Exit;
         end;
...
begin
ShowMessage('Автор проги Пичукан Айвар... В общем то неплохой программист. Соавтор проги Артем Пичукан...');
end;
...
begin
 ...
 // EXC.visible:=false;      //делаем Excel невидимым
    ...
    ProgBar:=70;
    ProgressBar1.Position := ProgBar ;
    Label2C:='доки оценщика и компании вставлены в WORD документ';
    Form1.Label2.Caption:=Label2C;
...
begin
 ...
 procedure IniFileCreate;
    ...
    IniSectionValue [1,3]:= 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОРЫ районы\'; //Как пример значения в секции INI файла
    ...
    //*******************************************
    IniSectionValue [3,2]:= 'ECONOMIC RF OBZOR';
    IniSectionValue [3,3]:= 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОР ЭКОНОМИКИ\';
   //*******************************************
    IniSectionValue [4,2]:= 'ECONOMIC SPB LO OBZOR';
    IniSectionValue [4,3]:= 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОР ЭКОНОМИКИ\';
   //*********************************************
    IniSectionValue [5,2]:= 'ECONOMIC SEGMENT OBZOR';
    IniSectionValue [5,3]:= 'Z:\GRAND NEVA\2014\ОБЗОРЫ\ОБЗОР СЕГМЕНТ РЫНКА\';
   //*********************************************
    IniSectionValue [6,2]:= 'DOCI OF COMPANY';
    IniSectionValue [6,3]:= 'Z:\GRAND NEVA\2014\КОМПАНИИ\ГРАНД НЕВА\';
...
begin
   ProgramName:='Blevantone 6.0 local unassigned crypt improve';
